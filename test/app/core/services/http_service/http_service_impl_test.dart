...
void main() {
  late Dio dioClientMock;
  late HttpService httpService;
  const baseUrl = 'https://jsonplaceholder.typicode.com';
  setUp(() {
    dioClientMock = HttpClientMock();
    httpService = HttpServiceImpl(
      baseUrl,
      httpClient: dioClientMock,
    );
  });
  ...
  group('HttpService ->', () {
    group('get ->', () {
      test(
        'Should return response with data and bytes of data then this method is called!',
        () async {
          when(
            () => dioClientMock.get(any()),
          ).thenAnswer(
            (_) async => Response(
              data: todosJson,
          ...
          final response = await httpService.get('/todos');

          expect(response.data, isNotNull);
          expect(response.data, isNotEmpty);
          expect(response.dataAsBytes, isNotEmpty);
        },
      );

      test(
        'Should thow AppException then no has internet',
        () async {
          when(
            () => dioClientMock.get(any()),
          ).thenThrow(
      ...
      test(
        'Should thow AppException then path isEmpty',
        () async {
          try {
...
class HttpClientMock extends Mock implements Dio {}

final todosJson = <Map<String, dynamic>>[
